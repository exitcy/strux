...
function readSidebarWidthFromCookie(): number {
  ...
  return clampSidebarWidth(Number(match[1]))
}
...
function SidebarProvider({
  ...
  React.useEffect(() => {
    setWidthState(readSidebarWidthFromCookie())
    // Enable width transitions only after the cookie width is applied.
  ...
  // Adds a keyboard shortcut to toggle the sidebar (mobile + collapsed modes).
  React.useEffect(() => {
...
function Sidebar({
  ...
  const { isMobile, state, openMobile, setOpenMobile, isResizing, widthReady } =
    useSidebar()
  const disableWidthTransition = isResizing || !widthReady
  ...
  if (collapsible === "none") {
    return (
      <div
        data-slot="sidebar"
        className={cn(
          "relative flex h-full w-[var(--sidebar-width)] flex-col bg-sidebar text-sidebar-foreground",
  ...
  return (
    ...
    >
      ...
      <div
        ...
        data-side={side}
        className={cn(
          "fixed inset-y-0 z-10 hidden h-svh w-[var(--sidebar-width)] data-[side=left]:left-0 data-[side=left]:group-data-[collapsible=offcanvas]:left-[calc(var(--sidebar-width)*-1)] data-[side=right]:right-0 data-[side=right]:group-data-[collapsible=offcanvas]:right-[calc(var(--sidebar-width)*-1)] md:flex",
          disableWidthTransition
            ? "transition-none"
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  const { setWidth, setIsResizing, isResizing, isMobile, width } = useSidebar()
  ...
  return (
    <button
      ...
      className={cn(
        "absolute inset-y-0 z-20 hidden w-2 transition-colors ease-linear sm:flex",
